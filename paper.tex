...
An Empirical Comparison Across Three Homogeneous Graph Datasets}
...
We deliberately omit Graph Convolutional Networks (GCN) \cite{kipf2017} in favor of GAT \cite{velickovic2018}, which offers greater architectural flexibility by learning edge-specific importance. While GAT can approximate mean aggregation with uniform weights, its ability to dynamically weigh neighbors makes it a more expressive and adaptive evolution of the GCN paradigm.
...
\textbf{GAT} \cite{velickovic2018} computes attention coefficients $\alpha_{ij}$ between pairs of neighbouring nodes and aggregates messages as a weighted sum $h_i' = \sum_j \alpha_{ij} W h_j$. This allows selective neighbour weighting, critical in heterogeneous co-purchase graphs. While uniform attention allows GAT to perform simple mean aggregation, its primary advantage lies in learning adaptive neighbor weights, providing a more flexible alternative to the fixed symmetric normalization used in standard GCNs \cite{kipf2017}. On MOLHIV we use GATv2Conv \cite{brody2022}, which conditions attention on both source and target node after transformation, enabling full expressive dynamic attention with native \texttt{edge\_dim} support.

\textbf{GraphSAGE} \cite{hamilton2017} aggregates neighbourhood features via a learned mean: $h_i' = W[\,h_i \,\|\, \text{mean}_{j \in \mathcal{N}(i)} h_j\,]$. Its inductive formulation handles variable-degree nodes robustly. On MOLHIV, GraphSAGE incorporates bond features via additive edge embeddings, ensuring a comparable baseline to GIN for evaluating aggregation-function expressivity.
...
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Best Config} & \textbf{Acc (\%)} & \textbf{F1} & \textbf{Time/Ep (s)} \\
\midrule
MLP     & ---                    & 84.55$\pm$0.41 & 0.800$\pm$0.006 & 0.014$\pm$0.001 \\
GAT     & lr=0.01,d=0.3,h=256    & 90.99$\pm$0.25 & 0.902$\pm$0.003 & 0.075$\pm$0.002 \\
GraphSAGE & lr=0.01,d=0.3,h=256  & 90.80$\pm$0.22 & 0.895$\pm$0.003 & 0.090$\pm$0.003 \\
GIN     & lr=0.001,d=0.3,h=256   & 88.26$\pm$0.51 & 0.865$\pm$0.006 & 0.095$\pm$0.003 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\midrule
Macro Avg Acc & 0.898 & 0.882 & 0.820 & --- \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/amazon_all_curves.jpeg}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/amazon_tsne.jpeg}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/molhiv_all_curves.jpeg}
\caption{Validation ROC-AUC trajectories across 100 training epochs for GIN, GAT, and GraphSAGE on OGBG-MOLHIV. GIN reaches its peak performance early in the training cycle, while GAT requires more extensive epochs to stabilize its attention mechanisms.}
\label{fig:molhiv_curves}
\end{figure}

Table~\ref{tab:molhiv} details the performance of the three models on the OGBG-MOLHIV binary graph classification task. GIN achieves the highest test ROC-AUC of 0.7842, leveraging GINEConv's native bond-feature integration and maximal WL-expressiveness. GraphSAGE reaches a competitive 0.7785---only 0.0057 ROC-AUC units below GIN---demonstrating that even with mean aggregation, the model effectively leverages both atom and bond information. GAT trails at 0.7761 despite GATv2Conv's dynamic attention over both atom features and bond attributes. The validation trajectories in Fig.~\ref{fig:molhiv_curves} reveal that while GIN and GraphSAGE plateau relatively early, GAT requires deeper training to fully learn robust attention mechanisms across complex molecular scaffolds.

As seen in Table~\ref{tab:molhiv}, all three models exhibit low precision (0.082--0.138) but moderate recall (0.512--0.684), reflecting the standard precision-recall trade-off under extreme class imbalance. The high recall prioritises identifying true active compounds at the cost of false positives---appropriate for a drug discovery screening context where missed actives (false negatives) carry a much higher opportunity cost than evaluating false leads.
...
As illustrated in Fig.~\ref{fig:molhiv_depth}, the best validation ROC-AUC peaks differently for each architecture: GIN peaks at depth 5 (0.8415), GAT at depth 8 (0.8267), and GraphSAGE at depth 8 (0.8309). While GAT and GraphSAGE validation performance peaks at depth 8 in this sweep, the final tuned configurations for the primary results (Table VII) utilize depth 5 and 3 respectively. These shallower depths were selected to balance the marginal ROC-AUC gains against the significantly lower parameter counts and training latencies detailed in Section IX, ensuring a more efficient screening profile. GIN's optimum at depth 5 reflects its sum aggregator's ability to accumulate significant structural evidence across the molecular scaffold before the onset of oversmoothing. GAT and GraphSAGE exhibit even greater depth robustness, reaching their maximum performance at depth 8; GAT's attention mechanism and GraphSAGE's mean aggregation effectively mitigate representational collapse at greater depths.

\begin{table}[t]
\caption{MOLHIV Ablation Study (test ROC-AUC at 100 epochs).\textsuperscript{a}}
\label{tab:ablation}
\centering
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{Full (100ep)} & \textbf{Atoms Only} & \textbf{Struct.\ Only} & \textbf{Chosen Depth ($L$)} \\
\midrule
GIN        & 0.7842$\pm$0.012 & 0.6427$\pm$0.012 & 0.6569$\pm$0.014 & 5 \\
GAT        & 0.7761$\pm$0.014 & 0.6069$\pm$0.012 & 0.6151$\pm$0.013 & 5 \\
GraphSAGE  & 0.7785$\pm$0.010 & 0.6517$\pm$0.011 & 0.5112$\pm$0.016 & 3 \\
\bottomrule
\end{tabular}%
}
\begin{minipage}{\columnwidth}\vspace{2pt}
\footnotesize \textsuperscript{a}All ablations re-trained for 100 epochs for controlled comparison. For these architectures, the 100-epoch baseline performance (Full) matches the 300-epoch tuned results in Table VII due to early convergence, ensuring a stable baseline for measuring ablated performance drops.
\end{minipage}
\end{table}
...
\caption{Validation ROC-AUC vs.\ number of GNN layers (depths 2, 5, 8) for all three architectures on OGBG-MOLHIV. GIN peaks at depth 5; GAT and GraphSAGE continue improving at greater depth, reflecting their respective mechanisms for managing over-squashing.}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/molhiv_tsne_all_models.jpeg}
...
\midrule
GIN        & 68.45 & 31.55 & 27.98 \\
GAT        & 63.54 & 36.46 & 20.46 \\
GraphSAGE  & 51.23 & 48.77 & 11.58 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\begin{tabular}{lccc}
\toprule
\textbf{Model} & \textbf{Parameters} & \textbf{Test ROC} & \textbf{Time/Epoch (s)} \\
\midrule
GIN        & 1,445,701 & 0.7842$\pm$0.012 & 11.82$\pm$0.2 \\
GAT        & 1,447,201 & 0.7761$\pm$0.014 & 20.76$\pm$0.5 \\
GraphSAGE  &   618,001 & 0.7785$\pm$0.010 & 4.75$\pm$0.1 \\
\bottomrule
\end{tabular}%
}
\end{table}
...
\caption{Efficiency comparison: test ROC-AUC vs.\ trainable parameters. GraphSAGE achieves competitive performance (0.7785 ROC) with only 618,001 parameters---less than half those of GIN or GAT.}
\label{fig:molhiv_eff}
\end{figure}

As quantified in Table~\ref{tab:misclass} and visualised in Fig.~\ref{fig:molhiv_tsne}, GIN achieves the highest active-molecule recall (68.45\% correct), while GraphSAGE maintains the lowest inactive misclassification rate (11.58\%), demonstrating a more conservative but precise classification profile compared to the aggressive sum-based GIN. The t-SNE projections demonstrate that active molecules form dense internal clusters surrounded by the massive inactive manifold; the overlap at the boundaries directly drives the false-positive rates. GAT identifies 63.54\% of active molecules correctly but generates a moderate inactive false positive rate (20.46\%), reflecting its slightly less precise structural differentiation at the cluster boundaries. GIN correctly classifies 68.45\% of active compounds but suffers the highest inactive misclassification rate (27.98\%), indicating its sum-based decision boundary is overly aggressive in claiming positives.

From an efficiency perspective, as plotted in Fig.~\ref{fig:molhiv_eff} alongside the metrics in Table~\ref{tab:efficiency}, GraphSAGE emerges as the clear winner: with only 618,001 trainable parameters---less than half of GIN's 1,445,701 or GAT's 1,447,201---it achieves a highly competitive 0.7785 ROC-AUC in just 4.75s per epoch, compared to GAT's 20.76s. GIN achieves the best overall ROC-AUC (0.7842) but requires substantially more parameters, cementing GraphSAGE as the recommended architecture for resource-constrained, large-scale molecular screening pipelines.
...
\caption{Left: rank heatmap across datasets (green=best, red=worst). Right: training time per epoch (s). Even when normalized per epoch, MOLHIV times ($\sim$20 s) dwarf node-level tasks ($<$0.1 s) due to the 41,127-graph throughput requirement.}
...
\textbf{GraphSAGE} ranks third overall (rank sum = 7): second on Amazon (90.80\%) and MOLHIV (0.7785 ROC-AUC), but third on Airports (kappa 0.3773). Its mean aggregator excels at large-scale node classification and molecular screening but fails to capture the fine ordinal structure of the Airports task, where the monotonic degree--activity relationship requires sum-type aggregation to accumulate hub-centric connectivity evidence. As highlighted in Fig.~\ref{fig:cross_rank_time}, the most striking GraphSAGE finding is its MOLHIV efficiency: it trails GIN by only 0.0057 ROC-AUC while using fewer than half as many parameters and demanding significantly less compute time.
...
\begin{itemize}
    ...
    \item \textbf{Choose GraphSAGE} for large-scale tasks with tight parameter or compute budgets; it delivers competitive performance (within 0.2 pp of GAT on Amazon, marginally outperforming GAT by 0.0024 ROC on MOLHIV) at less than half the parameter count.
