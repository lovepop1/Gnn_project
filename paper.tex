...
\caption{Validation ROC-AUC curves for GIN, GAT, and GraphSAGE on OGBG-MOLHIV. GIN achieves the highest validation ROC (0.8415) at 5-layer depth; GAT and GraphSAGE show stable trajectories across depths.}
...
As illustrated in Fig.~\ref{fig:molhiv_depth}, the best validation ROC-AUC peaks differently for each architecture: GIN peaks at depth 5 (0.8415), GAT at depth 8 (0.8267), and GraphSAGE at depth 8 (0.8309). GIN's optimum at depth 5 reflects its sum aggregator's ability to accumulate significant structural evidence across the molecular scaffold before the onset of oversmoothing. GAT and GraphSAGE exhibit even greater depth robustness, reaching their maximum performance at depth 8; GAT's attention mechanism and GraphSAGE's mean aggregation effectively mitigate representational collapse at greater depths.
...
\caption{Efficiency comparison: test ROC-AUC vs.\ trainable parameters and vs.\ wall-clock training time. GraphSAGE achieves competitive performance (0.7785 ROC) with only 618,001 parameters---less than half those of GIN or GAT---and the shortest training time of 4.75s per epoch.}
